...
app.get("/api", (req, res) => {
  res.json({
    status: "OK",
    message: "HourHire API is running!",
    endpoints: {
      test: "/api/test",
      jobs: "/api/jobs",
      "find-jobs": "/api/find-jobs (POST - upload resume)",
      docs: "/api-docs",
    },
  });
});
...
app.post("/api/find-jobs", upload.single("resume"), async (req, res) => {
  try {
    ...
    // ✅ Get search query from candidate or use default
    const searchQuery = candidate.searchQuery || "Full Stack AI Engineer India";
    ...
    console.error("Error in /api/find-jobs:", error);
    res.status(500).json({
      success: false,
